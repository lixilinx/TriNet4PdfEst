...
def mnist_performance( data_loader ):
    with torch.no_grad():
        ...
        for _, (data, _) in enumerate(data_loader):
            ...
            x = x.to(device)
            _, nll = U.MonoTriNet(x, Ws, flip=False)
            Nll += num_samples*nll
...
eta, step_size, num_epoch = 1e-4, 1e-4, 1000
...
for epoch in range(num_epoch):
    for _, (data, _) in enumerate(train_loader):
        ...
        grads = grad(nll, Ws)        
        with torch.no_grad():
            m1 = [0.9*a + 0.1*b + 0.1*eta*torch.sign(w) for (a, b, w) in zip(m1, grads, Ws)]
            m2 = [0.99*a + 0.01*(b + eta*torch.sign(w))**2 for (a, b, w) in zip(m2, grads, Ws)]
